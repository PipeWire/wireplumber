  ...
  if (type != G_TYPE_NONE && !link) {
    if (type == WP_TYPE_DEVICE || type == WP_TYPE_NODE) {
      ...
      if (create_child_cb) {
        ...
        g_variant_builder_init (&b, G_VARIANT_TYPE ("(ussa{ss}a{ss})"));
        g_variant_builder_add (&b, "u", id);
        g_variant_builder_add (&b, "s", type_str);
        g_variant_builder_add (&b, "s", spa_factory);
        g_variant_builder_add (&b, "@a{ss}",
            wp_properties_to_variant (properties));
        g_variant_builder_add (&b, "@a{ss}",
            wp_properties_to_variant (parent_props));
...
static void
activate_done (GObject * object, GAsyncResult * res, gpointer user_data)
{
  WpLuaGraphLoader * self = user_data;
  g_autoptr (GError) error = NULL;

  if (!wp_object_activate_finish (WP_OBJECT (object), res, &error))
    wp_warning_object (self, "%s", error->message);
}
...
create_object (WpLuaGraphLoader * self, GVariant * object_desc)
{
  ...
  if (vproperties)
    properties = wp_properties_new_from_variant (vproperties);
  ...
  switch (type) {
    ...
    case OBJECT_TYPE_DEVICE:
    case OBJECT_TYPE_NODE:
      wp_object_activate (WP_OBJECT (object),
          WP_PIPEWIRE_OBJECT_FEATURES_MINIMAL, NULL, activate_done, self);
      break;
    case OBJECT_TYPE_EXPORTED_NODE:
