...
struct _WpDefaultProfileClass
{
  ...
  void (*get_profile) (WpDefaultProfile *self, WpDevice *device,
...
static gint
find_device_profile (WpDevice *device, const gchar *lookup_name)
{
...
static void
wp_default_profile_get_profile (WpDefaultProfile *self, WpDevice *device,
    const gchar **curr_profile)
{
  WpDefaultProfilePrivate *priv =
      wp_default_profile_get_instance_private (self);
  ...
  /* Get the device name */
  dev_name = wp_proxy_get_property (WP_PROXY (device), PW_KEY_DEVICE_NAME);
...
static void
update_profile (WpDefaultProfile *self, WpDevice *device,
...
static void
on_device_profile_notified (WpProxy *device, GAsyncResult *res,
    WpDefaultProfile *self)
{
  ...
  /* Finish */
  profiles = wp_proxy_enum_params_finish (device, res, &error);
  if (error) {
  ...
  /* Update the profile */
  update_profile (self, WP_DEVICE (device), name);
}
    ...
    WpDefaultProfile *self)
{
  ...
  wp_proxy_enum_params (device, "Profile", NULL, NULL,
      (GAsyncReadyCallback) on_device_profile_notified, self);
}

static void
on_device_enum_profile_done (WpProxy *device, GAsyncResult *res,
    WpDefaultProfile *self)
{
  g_autoptr (WpIterator) profiles = NULL;
  g_autoptr (GError) error = NULL;
