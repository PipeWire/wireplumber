...
static const gchar * const loopback_node_names[] = {
  "test.export-context.capture", "test.export-context.playback", NULL
};

static const gchar * const local_node_names[] = {
  "test.export-context.localnode", NULL
};

static void
load_export_context (TestFixture * f)
{
...
/*
 * A LocalNode is created by a factory of the export context and exported over
 ...
 */
static void
test_local_node_in_export_context (TestFixture * f, gconstpointer data)
{
  ...
  node = wp_impl_node_new_from_pw_factory (f->base.core, "adapter",
      wp_properties_new (
          ...
          "node.name", "test.export-context.localnode",
          "media.class", "Audio/Sink",
          "audio.position", "[FL,FR]",
          NULL));
  g_assert_nonnull (node);

  /* it must live in the export context, not in the core's own one */
...
main (gint argc, gchar *argv[])
{
  ...
  g_test_add ("/wp/export-context/local-node",
      TestFixture, NULL,
      test_export_context_setup, test_local_node_in_export_context,
      test_export_context_teardown);
